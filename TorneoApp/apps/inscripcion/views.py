...
def export_data(request, level):
	...
	for jugador in jugadores:
		jugador_split = jugador.fullname.decode('utf-8').split(' ')
		content += str(jugador.pk)+';'+jugador.fullname.title()+';;;0;0;;AUT;;;;0;;0;;0;0;0;0,00;;;'+str(jugador.pk)+';'+((jugador_split[0]+' '+jugador_split[1]).title() if len(jugador_split) == 4 else jugador_split[0].title())+';'+((jugador_split[2]+' '+jugador_split[3]).title() if len(jugador_split) == 4 else (jugador_split[1]+' '+jugador_split[2]).title())+';\n'
